...
<script>
var profesores=[],lecciones=[],alumnos=[],clases=[],id_sesion  = -1;
	...
	id_sesion = document.getElementById('param').value;
			...
			else
			{
				...
				function(output)
				{
					...
					{
						$.post('Queries/getprofs.php' , { value: id_sesion },
						function(output)
						{
							...
							else
							{
								...
								for( i = 0 ; i < profesores.length; i++ )
								{
									...
									combo.options.add(new Option( getNombre(profesornombres[1],profesornombres[2],
										profesornombres[3],profesornombres[4]) , i));
								}
...
function cargarclases( idprof )
{
	...
	function(output)
	{
		if( output.substring(0,2) == "NO" )
		{
			alert("El profesor seleccionado no tiene ninguna clase creada");
			document.getElementById('leccion').disabled = true;
			document.getElementById('alumno').disabled = true;
			document.getElementById('clases').disabled = true;
		}
...
<table align="center" style=" padding-top: 120px;">
	<tr><td align="left">Profesor:</td><td align="left">Clases:</td></tr>
    <tr><td><SELECT id="prof" size="1" style="width:200px" onchange="
			var idprof = profesores[document.getElementById('prof').selectedIndex].split(',')[0];
			cargarclases(idprof);
    "> </SELECT></td><td><SELECT id="clases" size="1" style="width:200px" onchange="
			var idclase = clases[document.getElementById('clases').selectedIndex].split(',')[0];
			cargaralumnos(idclase);
    "> </SELECT></td></tr>
